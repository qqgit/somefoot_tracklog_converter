...
convert some foot trackjson data file (.txt) to plt format for OziExplorer use. 
some foot trackjson data url sample: 
Modified on 2016-10-17 by cqq
Added dir selection support and kml format (for Google Earth and Oruxmaps use) support.
...
class gpsConvFrame(wx.Frame):
    def __init__(self):
        ...
        TitleLbl = wx.StaticText(panel, -1, "Response ---> plt/kml")
        ...
        srcdbb = filebrowse.DirBrowseButton(
                                                panel,
                                                -1,
                                                size=(600, -1), 
                                                labelText="Response Source Dir: ", 
                                                changeCallback = self.srcdbbCallback 
                                            )
        tgtLbl = wx.StaticText(panel, -1, " Target File/Directory:  ")
        ...
        leftSizer.Add(srcdbb, 0, wx.EXPAND)
        
        asctgtSizer = wx.BoxSizer(wx.ALIGN_LEFT|wx.HORIZONTAL)
        asctgtSizer.Add(tgtLbl, 0,
                        wx.ALIGN_LEFT|wx.ALIGN_CENTER_VERTICAL)
        asctgtSizer.Add(tgtTxt, 1, wx.EXPAND|wx.ALL)
        
        leftSizer.Add(asctgtSizer, 0, wx.EXPAND|wx.ALL, 1)
    ...
    def srcfbbCallback(self, evt):
        ...
        if srcFileStr!=None and srcFileStr!="": self.srcdbb.SetValue("")
        self.tgtTxt.SetValue(tgtFileStr)
        
    def srcdbbCallback(self, evt):
        srcDirStr = evt.GetString()
        # parse source file name and generate default target file name, 
        # then set it to target control 
#         pathName,fileName = os.path.split(srcFileStr)
#         fileNameTrunk, fileNameExt = os.path.splitext(fileName)
...
#         self.tgtRevFileName = tgtRevFileStr
        ...
        if srcDirStr!=None and srcDirStr!="": self.srcfbb.SetValue("")
        self.tgtTxt.SetValue(srcDirStr)
        
    def OnconvertBtnClick(self, evt):
        srcFileName = self.srcfbb.GetValue()
        srcDirName = self.srcdbb.GetValue()
        tgtFileOrDir = self.tgtTxt.GetValue()
        ...
        if (os.path.isfile(srcFileName) == False and os.path.isdir(srcDirName) == False): 
            self.tgtTxt.SetValue(  "%s and %s do NOT exist!!!" % (srcFileName,srcDirName))
            return "File/Dir not exist!"
        if os.path.isfile(srcFileName) == True:
            # self.srcfbb widget is used, convert selected file
            self.writePLTFile(srcFileName,tgtFileOrDir)
            print("after writeTargetFile")
            self.writeKMLFile(srcFileName,tgtFileOrDir)
            print("after writeKMLFile")
            if os.path.isfile(tgtFileOrDir) == True: 
                self.tgtTxt.SetValue("plt file: %s has been created." % tgtFileOrDir)
            kmlFile = tgtFileOrDir.rstrip('.plt')+'.kml'
            if os.path.isfile(kmlFile) == True:
                self.tgtTxt.SetValue("kml file: %s has been created." % kmlFile)
                
        if os.path.isdir(srcDirName) == True:
            # self.srcdbb widget is used, loop recursively and convert all .txt files in selected dir
            track_ext = ['.txt']
            cnt = 0
            for parent, dirnames, filenames  in  os.walk(srcDirName):
                for  filename  in  filenames:
                    print(( " parent is: "   +  parent))
                    fullFileName = os.path.join(parent, filename)
                    fullFileNameTrunk, fileNameExt = os.path.splitext(fullFileName)
                    print(( " filename with full path : "   +  os.path.join(parent, filename)))
                    if fileNameExt.lower() in track_ext:
                        print("find track file %s in directory %s" % (filename,parent))
                        fullPLTFileName = fullFileNameTrunk + ".plt"
                        fullKMLFileName = fullFileNameTrunk + ".kml"
                        self.writePLTFile(fullFileName,fullPLTFileName)
                        print("fullPLTFileName: %s is writed" % fullPLTFileName)
                        self.writeKMLFile(fullFileName,fullKMLFileName)
                        print("fullKMLFileName: %s is writed" % fullKMLFileName)
                        cnt +=1
            self.tgtTxt.SetValue("%s track file has been found and coresponding plt and kml files are created" % cnt)
...
#       if sorted list of updated dict is exist, write it in "keyi = valuei" format  
    ...
    def writePLTFile(self,srcFileName=None, tgtFileName=None):
        '''
        read json data from response file (srcFileName), parse longitude, latitude and amplitude.
        then write data in plt format (for OZIExplorer) to tgtFileName
        format is: 
        Track File (.plt)
        ...
        pathName,fileName = os.path.split(tgtFileName)
        fileNameTrunk, fileNameExt = os.path.splitext(fileName)
        pltFileName = os.path.join(pathName,fileNameTrunk+".plt")
        ...
        print("pltFileName in writeKMLFile is: " + pltFileName)
        
        if utf8_json_data != None:
            f = codecs.open(pltFileName,mode="w")
            f.write("OziExplorer Track Point File Version 2.1\n")
            f.write("WGS 84\n")
            f.write("Altitude is in Feet\n")
            f.write("Reserved 3\n")
            f.write("0,2,255,ConvertedByQQ,0,0,2,8421376\n")
            f.write(str(len(utf8_json_data)) + "\n")
            # i[1] : Latitude - decimal degrees. 
            # i[2] : Longitude - decimal degrees.
            # 0    : code - 0 if normal, 1 if break in track line
            # i[3] : Altitude in feet (-777 if not valid)  1 m = 3.28084feet 
            # i[0] : TDateTime - 0 is 12/30/1899 12:00 am, convert to TDateTime is dummy
            #[[i[1],i[2],0,i[3]*3.28084,float(i[0])/40000.0] for i in utf8_json_data]
            for i in utf8_json_data:
                f.write(str(i[1])+','+str(i[2])+',0,'+str(i[3]*3.28084)+','+str(float(i[0])/40000.0)+',,'+'\n')
            f.close()      
...
#         from pykml.factory import GX_ElementMaker as GX
        ...
        outfile = open(kmlFileName,'wb') # there is no file function in python3, use open instead.
